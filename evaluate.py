...
def recognize(network_parameters, image_path, ctx, debug):
    ...
    *debug* this flag idicates if we are goig to show debug information
    ...
    normal = normalize_image(img, ctx[0])
...
def parse_args():
    ...
    parser.add_argument('--network-param', dest='network_param', help='Network parameter filename',
                        default='unet_best.params', type=str)
    parser.add_argument('--image', dest='img_path', help='Image filename to evaluate', default='data/input.png',
                        type=str)
...
if __name__ == '__main__':
    ...
    imwrite('./debug_out/%s_src.png' % (name), src)
    imwrite('./debug_out/%s_mask.png' % (name), mask)
    recoverImage(src, mask)
    imwrite('./debug_out/%s_recover.png' % (name), src)
